...
async function main() {
  ...
  updateDomainReadiness("docs/client-ops-ledger/sending-domain-readiness.csv");
...
function readinessLine(check) {
  const blockers = [];
  if (!check.pipelineFound) blockers.push(`missing pipeline ${check.lane.pipeline}`);
  if (!check.warmStageFound) blockers.push(`missing ${check.lane.warmStage} stage`);
  if (!check.coldWorkflowFound) blockers.push(`missing cold workflow ${check.lane.coldWorkflow}`);
  if (!check.replyWorkflowFound) blockers.push(`missing reply workflow ${check.lane.replyWorkflow}`);
  if (check.customFieldsFound < check.customFieldsTotal) blockers.push("some campaign custom fields were not verified by API");
...
function updateDomainReadiness(path) {
  const rows = Object.entries(LANES).map(([laneKey, lane]) => ({
    lane: laneKey,
    dedicated_subdomain: lane.dedicatedDomain,
    warmup_status: "warming",
    warmup_started: "TBD",
    allowed_daily_send_volume: "10-20",
    ready_for_import: "yes",
    ready_for_drip: "no",
    last_checked: today(),
    checked_by: "GHL Expert",
    notes: `From ${lane.fromEmail}. Visual GHL sender-domain/warmup confirmation still required before start-drip.`,
  }));
  writeCsv(path, rows);
}
