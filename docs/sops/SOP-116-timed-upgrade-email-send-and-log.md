# SOP 116 - Timed upgrade email send and log
...
Source of truth: `docs/sops/SOP-116-timed-upgrade-email-send-and-log.md`

## Purpose

Make the `Timed upgrade email send and log` workflow repeatable, auditable, and safe to delegate.

## Covered Master Map Rows

- Timed upgrade email send and log
...
Sales Manager approves angle and Account Manager approves timing

## Expected Output

Email sent, held, or call route logged

## Roles

| Role | Responsibility |
|---|---|
| Owner | Sales Rep owns the outcome and keeps this SOP current |
...
1. Confirm the prospect, source, current stage, offer lane, and allowed next action.
2. Check suppression, no-fit, existing-client, open-support, and custom-promise flags before replying.
3. Use approved language and keep the response short, helpful, and specific to the known context.
4. Send only after required approval exists for reports, pricing exceptions, live-send campaigns, or risky claims.
5. Log the message, status, next date, and next owner.

## Required Proof

- Expected output: Email sent, held, or call route logged
