...
Source of truth: `docs/sops/SOP-057-gbp-access-request.md`

## Purpose

Make the `GBP access request` workflow repeatable, auditable, and safe to delegate.
...
Client needs Google access setup
...
| Role | Responsibility |
|---|---|
| Owner | Account Manager owns the outcome and keeps this SOP current |
...
1. Confirm the client, plan, status, current blocker, and last client touch.
2. Check whether the client is blocked, upset, billing-flagged, or waiting on GMF before sending anything.
3. Use client-safe language: clear, short, specific, and tied to the next action.
4. Send, hold, or route the message based on approval and relationship timing.
5. Update the client hub/Monday record and assign the next owner.

## Required Proof

- Expected output: Client invite instructions
