# SOP 110 - Refund or billing exception

Status: Drafted
Version: 0.2
Owner: Manager/Mike
Reviewer: Auditor
Approver: Manager; Mike when material business risk applies
Effective date: Set when Active
Next review: Set when Active
Source of truth: `docs/sops/SOP-110-refund-or-billing-exception.md`

## Purpose

Make the `Refund or billing exception` workflow repeatable, auditable, and safe to delegate.
...
Refund, charge issue, exception

## Expected Output

Owner decision and Stripe action

## Roles

| Role | Responsibility |
|---|---|
| Owner | Manager/Mike owns the outcome and keeps this SOP current |
| Operator | Performs the work and reports gaps or blockers |
| Reviewer | Auditor checks proof, quality, and risk controls |
| Approver | Manager; Mike when material business risk applies approves activation or material changes |
...
2. Route routine work to the right role and keep routine status in Monday/Mission Control.
3. Escalate only approvals, access fixes, spend/cap changes, legal/billing/deliverability/reputation/customer-facing risk, or blockers agents cannot clear.
4. Record pass/watch/blocker status and next owner.
5. Follow up until the blocker is cleared or explicitly parked.

## Required Proof

- Expected output: Owner decision and Stripe action
- Work record or Monday item
- Date/time, owner, and status
- Approval note for financial or payment action
