# SOP 099 - Query fan-out topic coverage review

Status: Drafted
Version: 0.2
Owner: Reporter/Coach
Reviewer: Coach
Approver: SOP owner
Effective date: Set when Active
Next review: Set when Active
Source of truth: `docs/sops/SOP-099-query-fan-out-topic-coverage-review.md`

## Purpose

Make the `Query fan-out topic coverage review` workflow repeatable, auditable, and safe to delegate.

## Covered Master Map Rows

- Query fan-out topic coverage review
...
Human-useful topic gaps, not spam page list

## Roles

| Role | Responsibility |
|---|---|
| Owner | Reporter/Coach owns the outcome and keeps this SOP current |
...
1. Confirm report context, recipient type, source data, date range, and client/prospect lane.
2. Gather proof from approved records, public checks, dashboards, logs, and prior reports.
3. Build the artifact with plain-English findings, proof, blockers, and next action.
4. Route client/prospect-facing reports to Auditor before send.
5. Store/archive the report and update the delivery/status record.

## Required Proof

- Expected output: Human-useful topic gaps, not spam page list
